...
;;Funcion Heuristica que permite contar la distancia
  ...
  (DOTIMES (N 8)
    (SETQ DISTANCIA
    (+
     (ABS (- (POSX (POSITION  (+ 1 N)  ESTADO)) (POSX (POSITION (+ 1 N) META))))
     (ABS (- (POSY (POSITION  (+ 1 N) ESTADO)) (POSY (POSITION(+ 1 N) META))))
     DISTANCIA
   )
    )
)
  ...
  (LOOP
   ...
   (COND
    ...
    ((<= COSTO (CALCULACOSTO NTHNODO))
     (SETQ OPENSTACK (APPEND (FIRSTN INDEX OPENSTACK) (LIST NODO) (NTHCDR  INDEX OPENSTACK)))
     (RETURN-FROM ADDOPEN T)
     )
    )
   (SETQ INDEX (+ 1 INDEX))
